...
use faizdb_server::grpc::proto::{
    FaizDbService, HealthRequest, InsertRequest, QueryRequest, VectorSearchRequest,
};
...
async fn test_intensive_mongodb_wire_security_and_rbac() {
    ...
    // Setup accounts
    user_store
        .create_user("db_owner", "owner-pass-2026", Role::Admin)
        .unwrap();
    user_store
        .create_user("auditor", "audit-pass-2026", Role::ReadOnly)
        .unwrap();
    ...
    // -------------------------------------------------------------
    {
        ...
        assert_eq!(
            auth_bad_resp.get_str("codeName").unwrap(),
        ...
        // -------------------------------------------------------------
        let auth_good_resp = send_op_msg(
            &mut stream,
            doc! {
                "authenticate": 1,
                "user": "db_owner",
    ...
    // -------------------------------------------------------------
    {
        ...
        println!(
            "  🔐 [SEC-M10] SASL PLAIN authentication succeeded (conversationId=1, done=true)"
        );

        // Subsequent query succeeds
        let find_sasl = send_op_msg(
            &mut stream,
            doc! {
                "find": "products",
                "filter": doc! {}
...
async fn test_intensive_grpc_security_and_rbac() {
    ...
    user_store
        .create_user("grpc_admin", "admin-pass-999", Role::Admin)
        .unwrap();
    user_store
        .create_user("grpc_guest", "guest-pass-111", Role::ReadOnly)
        .unwrap();

    let admin_token = auth
        .generate_token("grpc_admin", Role::Admin, 3600)
        .unwrap();
    let guest_token = auth
        .generate_token("grpc_guest", Role::ReadOnly, 3600)
        .unwrap();
    ...
    println!(
        "  🛡️ [SEC-G2] Unauthenticated execute_query rejected: {}",
        err.message()
    );
    ...
    bad_jwt_req.metadata_mut().insert(
        "authorization",
        "Bearer eyJhbGciOi...tampered".parse().unwrap(),
    );
    let err = svc.execute_query(bad_jwt_req).await.unwrap_err();
    assert_eq!(err.code(), tonic::Code::Unauthenticated);
    println!(
        "  🛡️ [SEC-G3] Tampered JWT token rejected: {}",
        err.message()
    );
    ...
    valid_jwt_req.metadata_mut().insert(
        "authorization",
    ...
    basic_req.metadata_mut().insert(
        "authorization",
        format!("Basic {basic_creds}").parse().unwrap(),
    );
    ...
    bad_basic_req.metadata_mut().insert(
        "authorization",
    ...
    println!(
        "  🛡️ [SEC-G6] Invalid Basic Auth password rejected: {}",
        err_basic.message()
    );
    ...
    insert_guest_req.metadata_mut().insert(
        "authorization",
    ...
    println!(
        "  🛡️ [SEC-G7] ReadOnly user blocked from insert_documents by gRPC RBAC: {}",
        err_perm.message()
    );
    ...
    modify_query_req.metadata_mut().insert(
        "authorization",
        format!("Bearer {guest_token}").parse().unwrap(),
    );
    ...
    println!(
        "  🛡️ [SEC-G8] ReadOnly user blocked from DELETE query: {}",
        err_query.message()
    );
    ...
    let mut insert_admin_req = Request::new(InsertRequest {
        collection: "metrics".to_string(),
        documents_json: vec![
            r#"{"metric": "cpu", "value": 90, "vector": [0.1, 0.2, 0.3]}"#.to_string(),
        ],
    });
    insert_admin_req.metadata_mut().insert(
        "authorization",
        format!("Bearer {admin_token}").parse().unwrap(),
    );
...
async fn test_intensive_performance_benchmark_across_protocols() {
    ...
    user_store
        .create_user("bench_user", "bench-secret-2026", Role::Admin)
        .unwrap();
    let token = auth
        .generate_token("bench_user", Role::Admin, 3600)
        .unwrap();
    ...
    tokio::spawn(async move {
        let _ =
            faizdb_server::wire::postgres::run_postgres_server(&pg_addr_str, db_pg, store_pg).await;
    });
    ...
    println!(
        "    • Duration         : {:.3} ms",
        mongo_total_time.as_secs_f64() * 1000.0
    );
    println!("    • Throughput       : {:.1} ops/sec", mongo_tps);
    println!(
        "    • Latency p50      : {mongo_p50} µs ({:.3} ms)",
        mongo_p50 as f64 / 1000.0
    );
    println!(
        "    • Latency p90      : {mongo_p90} µs ({:.3} ms)",
        mongo_p90 as f64 / 1000.0
    );
    println!(
        "    • Latency p99      : {mongo_p99} µs ({:.3} ms)",
        mongo_p99 as f64 / 1000.0
    );
    ...
    println!(
        "    • Duration         : {:.3} ms",
        grpc_total_time.as_secs_f64() * 1000.0
    );
    println!("    • Throughput       : {:.1} ops/sec", grpc_tps);
    println!(
        "    • Latency p50      : {grpc_p50} µs ({:.3} ms)",
        grpc_p50 as f64 / 1000.0
    );
    println!(
        "    • Latency p90      : {grpc_p90} µs ({:.3} ms)",
        grpc_p90 as f64 / 1000.0
    );
    println!(
        "    • Latency p99      : {grpc_p99} µs ({:.3} ms)",
        grpc_p99 as f64 / 1000.0
    );
    ...
    let pg_start = Instant::now();
    {
        ...
        stream
            .write_all(&build_pg_startup_packet("bench_user"))
            .await
            .unwrap();
    ...
    println!(
        "    • End-to-end PG Handshake + Argon2id Auth Latency: {pg_handshake_us} µs ({:.2} ms)",
        pg_handshake_us as f64 / 1000.0
    );
    ...
    println!("=======================================================");
    println!(
        "  Gate                      | Throughput       | p50 Latency | p90 Latency | p99 Latency"
    );
    println!(
        "  --------------------------|------------------|-------------|-------------|------------"
    );
    println!(
        "  MongoDB Wire (Port 27017) | {:>10.1} ops/sec | {:>7} µs | {:>7} µs | {:>7} µs",
        mongo_tps, mongo_p50, mongo_p90, mongo_p99
    );
    println!(
        "  gRPC Gateway (Port 50051) | {:>10.1} ops/sec | {:>7} µs | {:>7} µs | {:>7} µs",
        grpc_tps, grpc_p50, grpc_p90, grpc_p99
    );
    println!(
        "  Postgres Wire Handshake   |        (Session) | {:>7} µs |           - |          -",
        pg_handshake_us
    );
    println!("=======================================================\n");
}
