...
pub async fn backup_create(
    ...
    let passphrase = body
        .and_then(|b| b.0.passphrase)
        .filter(|p| !p.trim().is_empty());

    let filename = if passphrase.is_some() {
        format!(
            "faizdb_snapshot_{}.enc.json",
            chrono::Utc::now().format("%Y%m%d_%H%M%S")
        )
    } else {
        format!(
            "faizdb_snapshot_{}.json",
            chrono::Utc::now().format("%Y%m%d_%H%M%S")
        )
    };
    ...
    if let Some(pass) = passphrase {
        ...
        match std::fs::write(
            &path,
            serde_json::to_string_pretty(&enc_wrapper).unwrap_or_default(),
        ) {
...
pub async fn backup_list() -> impl IntoResponse {
    ...
    if !backup_dir.exists() {
        return Json(ApiResponse::ok(
            Vec::<faizdb_core::backup::SnapshotManifest>::new(),
        ));
    }
                    ...
                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(&raw) {
                        if val.get("encrypted").and_then(|v| v.as_bool()) == Some(true) {
                            if let Some(m_val) = val.get("manifest") {
                                if let Ok(mut m) = serde_json::from_value::<
                                    faizdb_core::backup::SnapshotManifest,
                                >(m_val.clone())
                                {
...
pub async fn backup_restore(
            ...
            match latest {
                ...
                None => {
                    return (
                        ...
                        Json(ApiResponse::<()>::err("No backup snapshots found")),
                    )
                        .into_response()
                }
            }
    ...
    let archive_result: Result<faizdb_core::backup::SnapshotArchive, String> =
        if let Ok(val) = serde_json::from_str::<serde_json::Value>(&raw_content) {
            if val.get("encrypted").and_then(|v| v.as_bool()) == Some(true) {
                ...
                let cipher = faizdb_security::Cipher::from_passphrase(pass.as_str());
                let ciphertext: Vec<u8> = match val
                    .get("ciphertext")
                    .and_then(|c| serde_json::from_value(c.clone()).ok())
                {
                    ...
                    None => {
                        return (
                            StatusCode::BAD_REQUEST,
                            Json(ApiResponse::<()>::err("Corrupted encrypted snapshot")),
                        )
                            .into_response()
                    }
                };
                match cipher.decrypt(&faizdb_security::EncryptedData { ciphertext }) {
                    ...
                    Err(e) => {
                        return (
                            ...
                            Json(ApiResponse::<()>::err(format!("Invalid passphrase: {e}"))),
                        )
                            .into_response()
                    }
                }
    ...
    match archive_result {
        ...
        Err(e) => (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<()>::err(format!("Restore failed: {e}"))),
        )
            .into_response(),
    }
}
