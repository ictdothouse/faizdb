...
pub fn create_router(state: Arc<AppState>) -> Router {
    ...
    let public_routes = Router::new()
        ...
        .route("/v1/health/liveness", get(health::liveness_check))
        .route("/v1/health/readiness", get(health::readiness_check))
    ...
    let read_routes = Router::new()
        .route(
            ...
            get(collections::get_collection_documents),
        )
        .route(
        ...
        .route(
            ...
            post(collections::search_collection),
        )
        .route(
            "/v1/collections/{name}/ttl/stats",
            get(collections::collection_ttl_stats),
        )
        .route("/v1/subscribe", get(websocket::ws_global_subscribe))
        .route(
            "/v1/collections/{name}/watch",
            get(websocket::ws_collection_watch),
        )
    ...
    let write_routes = Router::new()
        ...
        .route(
            "/v1/collections/{name}/indexes",
            post(collections::create_collection_index),
        )
        .route(
            "/v1/collections/{name}/indexes/{field}",
            delete(collections::drop_collection_index),
        )
        .route(
        ...
        .route(
            "/v1/collections/{name}/import",
            post(collections::import_collection_data),
        )
        .route(
            "/v1/collections/{name}/aggregate",
            post(collections::aggregate_collection),
        )
        .route(
            "/v1/collections/{name}/ttl/purge",
            post(collections::collection_ttl_purge),
        )
        .route(
            "/v1/transaction/begin",
            post(collections::transaction_begin),
        )
        .route(
            "/v1/transaction/commit",
            post(collections::transaction_commit),
        )
        .route(
            "/v1/transaction/rollback",
            post(collections::transaction_rollback),
        )
    ...
    let admin_routes = Router::new()
        ...
        .route(
            "/v1/backup/schedule",
            get(backup::get_backup_schedule).post(backup::update_backup_schedule),
        )
    ...
    let cluster_routes = Router::new()
        ...
        .route(
            "/v1/cluster/failover",
            post(cluster::cluster_trigger_failover),
        )
        .route("/v1/cluster/raft/vote", post(cluster::raft_request_vote))
        .route(
            "/v1/cluster/raft/append",
        ...
        .layer(axum_middleware::from_fn(
            middleware::cluster_auth_middleware,
        ));
    ...
    Router::new()
        ...
        .layer(
            ServiceBuilder::new()
                ...
                .layer(axum_middleware::from_fn(
                    middleware::payload_size_middleware,
                ))
